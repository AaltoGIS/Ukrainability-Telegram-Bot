...
class LegacyBridge:
    ...
    def get_user_data(self, user_id, key=None, default=None):
        """Thread-safe access to self._user_data()."""
        try:
            with self._session_lock():
                ...
                return self._user_data()[user_id].get(key, default)
        except Exception as e:
    ...
    def set_user_data(self, user_id, key, value):
        """Thread-safe setting of self._user_data() values."""
        try:
            with self._session_lock():
                if user_id not in self._user_data():
                    self._user_data()[user_id] = {}
    ...
    def remove_user_data(self, user_id, key):
        """Thread-safe removal of self._user_data() keys."""
        try:
            with self._session_lock():
                if user_id in self._user_data() and key in self._user_data()[user_id]:
                    return self._user_data()[user_id].pop(key)
            return None
        except Exception as e:
            flow_logger.error(f"Error in remove_user_data: {e}")
            return None

    def get_user_profile(self, user_id, key=None, default=None):
        """Thread-safe access to self._user_profiles()."""
        try:
            with self._session_lock():
                ...
                return self._user_profiles()[user_id].get(key, default)
        except Exception as e:
    ...
    def set_user_profile(self, user_id, key, value):
        """Thread-safe setting of self._user_profiles() values."""
        try:
            with self._session_lock():
                if user_id not in self._user_profiles():
                    self._user_profiles()[user_id] = {}
