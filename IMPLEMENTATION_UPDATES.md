...
- Splitting the remaining `_legacy.py` responsibilities into per-question modules under `survey/questions/` and removing the temporary `HandlerRegistry`. The next Phase 5 PR should migrate an actual survey section, starting with welcome/language/location.
